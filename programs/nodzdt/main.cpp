/**
 *  @file
 *  @copyright defined in zdtio/LICENSE.txt
 */
#include <appbase/application.hpp>

#include <zdtio/chain_plugin/chain_plugin.hpp>
#include <zdtio/http_plugin/http_plugin.hpp>
#include <zdtio/net_plugin/net_plugin.hpp>
#include <zdtio/producer_plugin/producer_plugin.hpp>
...
{
   try {
      app().set_version(zdtio::nodzdt::config::version);

      auto root = fc::app_path();
      app().set_default_data_dir(root / "zdtio/nodzdt/data" );
      app().set_default_config_dir(root / "zdtio/nodzdt/config" );
      ...
      initialize_logging();
      ilog("nodzdt version ${ver}", ("ver", app().version_string()));
      ilog("zdtio root is ${root}", ("root", root.string()));
      ilog("nodzdt using configuration file ${c}", ("c", app().full_config_file_path().string()));
      ilog("nodzdt data directory is ${d}", ("d", app().data_dir().string()));
      app().startup();
      app().exec();
   ...
   ilog("nodzdt successfully exiting");
   return SUCCESS;
}
