 ...
 *  @defgroup zdtclienttool ZDTIO Command Line Client Reference
 *  @brief Tool for sending transactions and querying state from @ref nodzdt
 *  @ingroup zdtclienttool
 */

/**
  @defgroup zdtclienttool

  @section intro Introduction to clzdt

  `clzdt` is a command line tool that interfaces with the REST api exposed by @ref nodzdt. In order to use `clzdt` you will need to
  have a local copy of `nodzdt` running and configured to load the 'zdtio::chain_api_plugin'.

   clzdt contains documentation for all of its commands. For a list of all commands known to clzdt, simply run it with no arguments:
```
$ ./clzdt
Command Line Interface to ZDTIO Client
Usage: programs/clzdt/clzdt [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where nodzdt is running
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where kzdtd is running
...
#include <zdtio/chain/name.hpp>
#include <zdtio/chain/config.hpp>
#include <zdtio/chain/wast_to_wasm.hpp>
#include <zdtio/chain/trace.hpp>
#include <zdtio/chain_plugin/chain_plugin.hpp>
...
using namespace zdtio::chain;
using namespace zdtio::client::help;
using namespace zdtio::client::http;
using namespace zdtio::client::localize;
using namespace zdtio::client::config;
using namespace boost::filesystem;
using auth_type = fc::static_variant<public_key_type, permission_level>;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define ZDTC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
//copy pasta from kzdtd's main.cpp
...
string default_wallet_url = "unix://" + (determine_home_directory() / "zdtio-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
zdtio::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      auto sp = std::make_unique<zdtio::client::http::connection_param>(context, parse_url(url) + path, no_verify ? false : true, headers);
      return zdtio::client::http::do_http_call(*sp, fc::variant(v), print_request, print_response );
   }
   catch(boost::system::system_error& e) {
      if(url == ::url)
         std::cerr << localized("Failed to connect to nodzdt at ${u}; is nodzdt running?", ("u", url)) << std::endl;
      else if(url == ::wallet_url)
         std::cerr << localized("Failed to connect to kzdtd at ${u}; is kzdtd running?", ("u", url)) << std::endl;
...
zdtio::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<zdtio::chain_apis::read_only::get_info_results>();
}
...
fc::variant push_transaction( signed_transaction& trx, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   if (trx.signatures.size() == 0) { // #5445 can't change txn content if already signed
      ...
      } ZDT_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
auto abi_serializer_resolver = [](const name& account) -> optional<abi_serializer> {
   ...
   if ( it == abi_cache.end() ) {
      ...
      auto abi_results = result.as<zdtio::chain_apis::read_only::get_abi_results>();
...
bytes json_or_file_to_bin( const account_name& account, const action_name& action, const string& data_or_filename ) {
   ...
   if( !data_or_filename.empty() ) {
      ...
      } ZDT_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data_or_filename));
   }
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } ZDT_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "ZDT") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } ZDT_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      ZDT_ASSERT( zdtio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( account_name code, const string& s ) {
   static map< pair<account_name, zdtio::chain::symbol_code>, zdtio::chain::symbol> cache;
   auto a = asset::from_string( s );
   zdtio::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<zdtio::chain_apis::read_only::get_currency_stats_result>();
         auto p = cache.emplace( make_pair( code, sym ), result.max_supply.get_symbol() );
         it = p.first;
      } else {
         ZDT_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}", ("s", sym_str)("c", code));
      }
   }
   ...
   if ( a.decimals() < expected_symbol.decimals() ) {
      ...
      ZDT_THROW(symbol_type_exception, "Too many decimal digits in ${a}, only ${d} supported", ("a", a)("d", expected_symbol.decimals()));
   } // else precision matches
   return a;
}

inline asset to_asset( const string& s ) {
   return to_asset( N(zdtio.token), s );
}
...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->add_flag("--add-code", add_code, localized("[code] add '${code}' permission to specified permission authority", ("code", name(config::zdtio_code_name))));
      permissions->add_flag("--remove-code", remove_code, localized("[code] remove '${code}' permission from specified permission authority", ("code", name(config::zdtio_code_name))));

      add_standard_transaction_options(permissions, "account@active");

      permissions->set_callback([this] {
         ZDTC_ASSERT( !(add_code && remove_code), "ERROR: Either --add-code or --remove-code can be set" );
         ZDTC_ASSERT( (add_code ^ remove_code) || !authority_json_or_file.empty(), "ERROR: authority should be specified unless add or remove code permission" );
         ...
         if ( need_parent || need_auth ) {
            ...
            auto res = json.as<zdtio::chain_apis::read_only::get_account_results>();
            ...
            if ( need_auth ) {
               ...
               auto code_name = name(config::zdtio_code_name);
...
void try_local_port(uint32_t duration) {
   ...
   while ( !local_port_used()) {
      if (duration_cast<std::chrono::milliseconds>( system_clock::now().time_since_epoch()).count() - start_time > duration ) {
         std::cerr << "Unable to connect to kzdtd, if kzdtd is running please kill the process and try again.\n";
         throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to kzdtd")});
      }
   }
}

void ensure_kzdtd_running(CLI::App* app) {
    if (no_auto_kzdtd)
        return;
    // get, version, net do not require kzdtd
    ...
    // This extra check is necessary when running clzdt like this: ./clzdt ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if clzdt and kzdtd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kzdtd").append(key_store_executable_name);
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child kzdt(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (kzdt.running()) {
        ...
        std::cerr << "No wallet service listening on "
                  << ". Cannot automatically start kzdtd because kzdtd was not found." << std::endl;
    }
}
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->set_callback([this] {
         ...
         } ZDT_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
...
struct create_account_subcommand {
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         ...
         createAccount->add_option("--buy-ram", buy_ram_zdt,
      ...
      createAccount->set_callback([this] {
            ...
            if( owner_key_str.find('@') != string::npos ) {
               ...
               } ZDT_RETHROW_EXCEPTIONS( explained_exception, "Invalid owner permission level: ${permission}", ("permission", owner_key_str) )
            } else {
               try {
                  owner = public_key_type(owner_key_str);
               } ZDT_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str) );
            }
            ...
            if( active_key_str.empty() ) {
               ...
               } ZDT_RETHROW_EXCEPTIONS( explained_exception, "Invalid active permission level: ${permission}", ("permission", active_key_str) )
            } else {
               try {
                  active = public_key_type(active_key_str);
               } ZDT_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str) );
            }

            auto create = create_newaccount(creator, account_name, owner, active);
            if (!simple) {
               ZDTC_ASSERT( buy_ram_zdt.size() || buy_ram_bytes_in_kbytes || buy_ram_bytes, "ERROR: One of --buy-ram, --buy-ram-kbytes or --buy-ram-bytes should have non-zero value" );
               ZDTC_ASSERT( !buy_ram_bytes_in_kbytes || !buy_ram_bytes, "ERROR: --buy-ram-kbytes and --buy-ram-bytes cannot be set at the same time" );
               action buyram = !buy_ram_zdt.empty() ? create_buyram(creator, account_name, to_asset(buy_ram_zdt))
...
struct vote_producers_subcommand {
   string voter_str;
   vector<zdtio::name> producer_names;
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so clzdt can still work with old buggy nodzdt versions
                               // Change to voter.value when clzdt no longer needs to support nodzdt versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<zdtio::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when clzdt no longer needs to support nodzdt versions older than 1.5.0
            ...
            ZDT_ASSERT( 1 == res.rows.size(), multiple_voter_info, "More than one voter_info for account" );
            auto prod_vars = res.rows[0]["producers"].get_array();
            vector<zdtio::name> prods;
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<zdtio::chain_apis::read_only::get_producers_result>();
...
struct get_transaction_id_subcommand {
   ...
   get_transaction_id_subcommand(CLI::App* actionRoot) {
      ...
      get_transaction_id->set_callback([&] {
         ...
         } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_check))
      });
   }
};
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->set_callback([this] {
         ...
         ZDTC_ASSERT( !(buy_ram_amount.size()) || !buy_ram_bytes, "ERROR: --buyram and --buy-ram-bytes cannot be set at the same time" );
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "zdtio")("scope", "zdtio")("table", "namebids")
                               ("lower_bound", newname.value)
                               ("upper_bound", newname.value + 1)
                               // Less than ideal upper_bound usage preserved so clzdt can still work with old buggy nodzdt versions
                               // Change to newname.value when clzdt no longer needs to support nodzdt versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         }
         auto result = rawResult.as<zdtio::chain_apis::read_only::get_table_rows_result>();
         // Condition in if statement below can simply be res.rows.empty() when clzdt no longer needs to support nodzdt versions older than 1.5.0
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->set_callback([this] {
         ZDTC_ASSERT( !kbytes || !bytes, "ERROR: --kbytes and --bytes cannot be set at the same time" );
         if (kbytes || bytes) {
...
void get_account( const string& accountName, const string& coresym, bool json_format ) {
   ...
   if (!json_format) {
      ...
      unordered_map<name, zdtio::chain_apis::permission> cache;
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of nodzdt responding to the request is different than core symbol built into clzdt
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'zdtio::refund' action)\n";
            } else {
...
int main( int argc, char** argv ) {
   ...
   context = zdtio::client::http::create_http_context();
   wallet_url = default_wallet_url;

   CLI::App app{"Command Line Interface to ZDTIO Client"};
   app.require_subcommand();
   app.add_option( "-H,--host", obsoleted_option_host_port, localized("the host where nodzdt is running") )->group("hidden");
   app.add_option( "-p,--port", obsoleted_option_host_port, localized("the port where nodzdt is running") )->group("hidden");
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where kzdtd is running") )->group("hidden");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where kzdtd is running") )->group("hidden");

   app.add_option( "-u,--url", url, localized("the http/https URL where nodzdt is running"), true );
   app.add_option( "--wallet-url", wallet_url, localized("the http/https URL where kzdtd is running"), true );

   app.add_option( "-r,--header", header_opt_callback, localized("pass specific HTTP header; repeat this option to pass multiple headers"));
   app.add_flag( "-n,--no-verify", no_verify, localized("don't verify peer certificate when using HTTPS"));
   app.add_flag( "--no-auto-kzdtd", no_auto_kzdtd, localized("don't automatically launch a kzdtd if one is not currently running"));
   app.set_callback([&app]{ ensure_kzdtd_running(&app);});
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", zdtio::client::config::version_str)) << std::endl;
   });
   ...
   pack_transaction->add_flag("--pack-action-data", pack_action_data_flag, localized("Pack all action data within transaction, needs interaction with nodzdt"));
   pack_transaction->set_callback([&] {
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string( plain_signed_transaction_json );
      } ZDT_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse plain transaction JSON '${data}'", ("data", plain_signed_transaction_json))
      if( pack_action_data_flag ) {
         ...
         } ZDT_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to convert transaction, --pack-action-data likely needed" )
      }
   });
   ...
   unpack_transaction->add_flag("--unpack-action-data", unpack_action_data_flag, localized("Unpack all action data within transaction, needs interaction with nodzdt"));
   unpack_transaction->set_callback([&] {
      ...
      } ZDT_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse packed transaction JSON '${data}'", ("data", packed_transaction_json))
   ...
   pack_action_data->set_callback([&] {
      ...
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse unpacked action data JSON")
   ...
   unpack_action_data->set_callback([&] {
      ZDT_ASSERT( packed_action_data_string.size() >= 2, transaction_type_exception, "No packed_action_data found" );
   ...
   getCode->set_callback([&] {
      ...
      try {
         ...
         //see if this is an old nodzdt that doesn't support get_raw_code_and_abi
         ...
         if(code_as_wasm) {
            ...
            std::cout << localized("Warning: communicating to older nodzdt which returns malformed binary wasm") << std::endl;
         }
         else
   ...
   getAccounts->set_callback([&] {
      ...
      } ZDT_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
   ...
   getTransactions->set_callback([&] {
      ...
      if (skip_seq_str.empty()) {
         ...
         } ZDT_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Skip Seq: ${skip_seq}", ("skip_seq", skip_seq_str))
         if (num_seq_str.empty()) {
            ...
            } ZDT_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Num Seq: ${num_seq}", ("num_seq", num_seq_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        ZDT_ASSERT( !wasm.empty(), wasm_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto set_abi_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        ZDT_ASSERT( fc::exists( abiPath ), abi_file_not_found, "no abi file found ${f}", ("f", abiPath)  );
      ...
      if (!duplicate) {
         ...
         } ZDT_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
         if ( shouldSend ) {
   ...
   contractSubcommand->set_callback([&] {
      if(!contract_clear) ZDT_ASSERT( !contractPath.empty(), contract_exception, " contract-dir is null ", ("f", contractPath) );
      shouldSend = false;
      set_code_callback();
      set_abi_callback();
      if (actions.size()) {
   ...
   createWallet->set_callback([&wallet_name, &password_file, &print_console] {
      ZDTC_ASSERT( !password_file.empty() ^ print_console, "ERROR: Either indicate a file using \"--file\" or pass \"--to-console\"" );
      ZDTC_ASSERT( password_file.empty() || !std::ofstream(password_file.c_str()).fail(), "ERROR: Failed to create file in specified path" );
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         ZDT_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   removeKeyWallet->set_callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
      ...
      try {
         ...
         ZDT_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
      }
   ...
   auto stopKzdtd = wallet->add_subcommand("stop", localized("Stop kzdtd."), false);
   stopKzdtd->set_callback([] {
      const auto& v = call(wallet_url, kzdtd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success kzdtd responds with empty object
   ...
   sign->set_callback([&] {
      ...
      if( str_chain_id.size() == 0 ) {
         ilog( "grabbing chain_id from nodzdt" );
         auto info = get_info();
   ...
   actionsSubcommand->set_callback([&] {
      ...
      if( !data.empty() ) {
         ...
         } ZDT_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data))
      }
   ...
   trxSubcommand->set_callback([&] {
      ...
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_push))
      try {
         signed_transaction trx = trx_var.as<signed_transaction>();
   ...
   trxsSubcommand->set_callback([&] {
      ...
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trxsJson))
   ...
   propose_action->set_callback([&] {
      ...
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",requested_perm))
      fc::variant transaction_perm_var;
      try {
         transaction_perm_var = json_from_file_or_string(transaction_perm);
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm))
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string(proposed_transaction);
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      ...
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            ZDT_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "zdtio.msig", "propose", variant_to_bin( N(zdtio.msig), N(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so clzdt can still work with old buggy nodzdt versions
                                 // Change to name(proposal_name).value when clzdt no longer needs to support nodzdt versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when clzdt no longer needs to support nodzdt versions older than 1.5.0
      ...
      std::map<zdtio::account_name, std::pair<fc::time_point, vector<decltype(all_approvals)::iterator>>>  provided_approvers;
      ...
      if( show_approvals_in_multisig_review ) {
         ...
         if( new_multisig ) {
            for( auto& a : provided_approvers ) {
               ...
               if( rows4.empty() || rows4[0].get_object()["account"].as<zdtio::name>() != a.first ) {
                  continue;
               }
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",perm))
      ...
      send_actions({chain::action{accountPermissions, "zdtio.msig", action, variant_to_bin( N(zdtio.msig), action, args ) }});
   };
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "zdtio.msig", "invalidate", variant_to_bin( N(zdtio.msig), "invalidate", args ) }});
   });
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            ZDT_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "zdtio.msig", "cancel", variant_to_bin( N(zdtio.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            ZDT_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "zdtio.msig", "exec", variant_to_bin( N(zdtio.msig), N(exec), args ) }});
      }
   );
   ...
   wrap_exec->set_callback([&] {
      ...
      } ZDT_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_exec))
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send zdtio.system contract action to the blockchain."), false);
