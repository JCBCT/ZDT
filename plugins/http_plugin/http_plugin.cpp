...
#include <zdtio/http_plugin/local_endpoint.hpp>
...
namespace zdtio {
      ...
      public:
         ...
         ssl_context_ptr on_tls_init(websocketpp::connection_hdl hdl) {
            ...
            try {
               ...
               if (!ecdh)
                  ZDT_THROW(chain::http_exception, "Failed to set NID_secp384r1");
               if(SSL_CTX_set_tmp_ecdh(ctx->native_handle(), (EC_KEY*)ecdh) != 1)
                  ZDT_THROW(chain::http_exception, "Failed to set ECDH PFS");
               ...
               if(SSL_CTX_set_cipher_list(ctx->native_handle(), \
                  ...
                  ZDT_THROW(chain::http_exception, "Failed to set HTTPS cipher list");
   ...
   void http_plugin::plugin_initialize(const variables_map& options) {
      try {
         ...
         ZDT_ASSERT( my->thread_pool_size > 0, chain::plugin_config_exception,
