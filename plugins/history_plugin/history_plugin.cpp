#include <zdtio/history_plugin/history_plugin.hpp>
#include <zdtio/history_plugin/account_control_history_object.hpp>
#include <zdtio/history_plugin/public_key_history_object.hpp>
...
CHAINBASE_SET_INDEX_TYPE(zdtio::account_history_object, zdtio::account_history_index)
CHAINBASE_SET_INDEX_TYPE(zdtio::action_history_object, zdtio::action_history_index)
...
namespace zdtio {
      ...
      try {
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               if( s == "*" || s == "\"*\"" ) {
                  ...
                  wlog( "--filter-on * enabled. This can fill shared_mem, causing nodzdt to stop." );
                  break;
               }
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               ZDT_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-on", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               ZDT_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
         ...
         if( options.count( "filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               ZDT_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-out", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               ZDT_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
         ...
         ZDT_ASSERT( my->chain_plug, chain::missing_chain_plugin_exception, ""  );
   ...
   namespace history_apis {
      read_only::get_actions_result read_only::get_actions( const read_only::get_actions_params& params )const {
        ...
        ZDT_ASSERT( end >= start, chain::plugin_exception, "end position is earlier than start position" );
      ...
      read_only::get_transaction_result read_only::get_transaction( const read_only::get_transaction_params& p )const {
         ...
         } ZDT_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", p.id))
         ...
         if( !in_history && !p.block_num_hint ) {
            ZDT_THROW(tx_not_found, "Transaction ${id} not found in history and no block hint was given", ("id",p.id));
         }
         ...
         if( in_history ) {
            ...
            if (!found) {
               ZDT_THROW(tx_not_found, "Transaction ${id} not found in history or in block number ${n}", ("id",p.id)("n", *p.block_num_hint));
            }
         }
