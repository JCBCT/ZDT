...
#include <zdtio/bnet_plugin/bnet_plugin.hpp>
...
using zdtio::public_key_type;
using zdtio::chain_id_type;
using zdtio::block_id_type;
using zdtio::block_timestamp_type;
using std::string;
using zdtio::sha256;
using zdtio::signed_block_ptr;
using zdtio::packed_transaction_ptr;
using std::vector;
...
namespace zdtio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              ZDT_THROW(block_validate_exception, "bad block" );
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           ZDT_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         ZDT_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
   ...
   void session::on( const packed_transaction_ptr& p ) {
      ...
      if (!p) {
        ...
        ZDT_THROW(transaction_exception, "bad transaction");
      }
