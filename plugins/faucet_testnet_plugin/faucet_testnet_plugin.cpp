 ...
 */
#include <zdtio/faucet_testnet_plugin/faucet_testnet_plugin.hpp>
...
namespace zdtio { namespace detail {
...
FC_REFLECT(zdtio::detail::faucet_testnet_empty, );
FC_REFLECT(zdtio::detail::faucet_testnet_keys, (owner)(active));
FC_REFLECT(zdtio::detail::faucet_testnet_create_account_params, (account)(keys));
FC_REFLECT(zdtio::detail::faucet_testnet_create_account_alternates_response, (alternates)(message));
FC_REFLECT(zdtio::detail::faucet_testnet_create_account_rate_limited_response, (message));
...
struct faucet_testnet_plugin_impl {
   ...
   results_pair find_alternates(const std::string& new_account_name) {
      ...
      const zdtio::detail::faucet_testnet_create_account_alternates_response response{
   ...
   results_pair create_account(const std::string& new_account_name, const fc::crypto::public_key& owner_pub_key, const fc::crypto::public_key& active_pub_key) {
      ...
      ZDT_ASSERT(creating_account != nullptr, transaction_exception,
      ...
      {
         zdtio::detail::faucet_testnet_create_account_rate_limited_response response{
      ...
      return std::make_pair(account_created, fc::variant(zdtio::detail::faucet_testnet_empty()));
   }

   results_pair create_faucet_account(const std::string& body) {
      const zdtio::detail::faucet_testnet_create_account_params params = fc::json::from_string(body).as<zdtio::detail::faucet_testnet_create_account_params>();
...
const key_pair faucet_testnet_plugin_impl::_default_key_pair = {"ZDT6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"};
