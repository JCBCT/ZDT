...
#include <zdtio/chain/permission_object.hpp>
#include <zdtio/chain/permission_link_object.hpp>
...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::zdtio_code_name ) // virtual zdtio.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         ZDT_THROW( action_validate_exception,
 ...
 */
void apply_zdtio_newaccount(apply_context& context) {
   auto create = context.act.data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::zdtio_auth_scope );
   auto& authorization = context.control.get_mutable_authorization_manager();

   ZDT_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   ZDT_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   ZDT_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   ZDT_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );

   // Check if the creator is privileged
   const auto &creator = db.get<account_object, by_name>(create.creator);
   if( !creator.privileged ) {
      ZDT_ASSERT( name_str.find( "zdtio." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'zdtio.'" );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   ZDT_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_zdtio_setcode(apply_context& context) {
   ...
   ZDT_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   ZDT_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );
   ...
   ZDT_ASSERT( account.code_version != code_id, set_exact_code, "contract is already running this version of code" );
...
void apply_zdtio_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_zdtio_updateauth(apply_context& context) {
   ...
   ZDT_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   ZDT_ASSERT( update.permission.to_string().find( "zdtio." ) != 0, action_validate_exception,
               "Permission names that start with 'zdtio.' are reserved" );
   ZDT_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   ZDT_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      ZDT_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      ZDT_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      ZDT_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      ZDT_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      ZDT_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_zdtio_deleteauth(apply_context& context) {
//   context.require_write_lock( config::zdtio_auth_scope );
   ...
   ZDT_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   ZDT_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
   ...
   { // Check for links to this permission
      ...
      ZDT_ASSERT(range.first == range.second, action_validate_exception,
...
void apply_zdtio_linkauth(apply_context& context) {
//   context.require_write_lock( config::zdtio_auth_scope );

   auto requirement = context.act.data_as<linkauth>();
   try {
      ZDT_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      ZDT_ASSERT(account != nullptr, account_query_exception,
      ...
      ZDT_ASSERT(code != nullptr, account_query_exception,
                 "Failed to retrieve code for account: ${account}", ("account", requirement.code));
      if( requirement.requirement != config::zdtio_any_name ) {
         const auto *permission = db.find<permission_object, by_name>(requirement.requirement);
         ZDT_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         ZDT_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_zdtio_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::zdtio_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   ZDT_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_zdtio_canceldelay(apply_context& context) {
