...
namespace zdtio { namespace chain {
    ...
    *  In the context of zdtio, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         ZDT_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         ZDT_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( zdtio::chain::permission_level, (actor)(permission) )
FC_REFLECT( zdtio::chain::action, (account)(name)(authorization)(data) )
