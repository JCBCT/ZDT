...
namespace zdtio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(zdtio);
const static uint64_t null_account_name      = N(zdtio.null);
const static uint64_t producers_account_name = N(zdtio.prods);
...
const static uint64_t zdtio_auth_scope       = N(zdtio.auth);
const static uint64_t zdtio_all_scope        = N(zdtio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t zdtio_any_name = N(zdtio.any);
const static uint64_t zdtio_code_name = N(zdtio.code);
...
const static zdtio::chain::wasm_interface::vm_type default_wasm_runtime = zdtio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace zdtio::chain::config

constexpr uint64_t ZDT_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / zdtio::chain::config::percent_100;
}

template<typename Number>
Number ZDT_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + zdtio::chain::config::percent_100 - zdtio::chain::config::percent_1)  / zdtio::chain::config::percent_100;
}
