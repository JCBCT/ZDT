...
#define N(X) zdtio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<zdtio::chain::name> : private hash<uint64_t> {
      typedef zdtio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const zdtio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, zdtio::chain::name& check);
} // fc


FC_REFLECT( zdtio::chain::name, (value) )
