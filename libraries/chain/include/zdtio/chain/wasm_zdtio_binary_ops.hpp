...
namespace zdtio { namespace chain { namespace wasm_ops {
...
struct field_specific_params {
   ...
   static void pack(instruction_stream* stream, Field& f) { return zdtio::chain::wasm_ops::pack(stream, f); }
   static auto to_string(Field& f) { return std::string(" ")+
                                       zdtio::chain::wasm_ops::to_string(f); }
};
template <>
...
template <class Op_Types>
struct ZDTIO_OperatorDecoderStream
{
   ZDTIO_OperatorDecoderStream(const std::vector<U8>& codeBytes)
   ...
   instr* decodeOp() {
      ZDT_ASSERT(nextByte + sizeof(IR::Opcode) <= end, wasm_exception, "");
      ...
      #define VISIT_OPCODE(opcode,name,nameString,Imm,...) \
         case IR::Opcode::name: \
         { \
            ZDT_ASSERT(nextByte + sizeof(IR::OpcodeAndImm<IR::Imm>) <= end, wasm_exception, ""); \
...
template <class Op_Types>
const std::vector<instr*>* ZDTIO_OperatorDecoderStream<Op_Types>::_cached_ops;

}}} // namespace zdtio, chain, wasm_ops

FC_REFLECT_TEMPLATE( (typename T), zdtio::chain::wasm_ops::block< T >, (code)(rt) )
FC_REFLECT( zdtio::chain::wasm_ops::memarg, (a)(o) )
FC_REFLECT( zdtio::chain::wasm_ops::blocktype, (result) )
FC_REFLECT( zdtio::chain::wasm_ops::memoryoptype, (end) )
