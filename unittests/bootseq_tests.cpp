...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(zdtio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create zdtio.msig and zdtio.token
        create_accounts({N(zdtio.msig), N(zdtio.token), N(zdtio.ram), N(zdtio.ramfee), N(zdtio.stake), N(zdtio.vpay), N(zdtio.bpay), N(zdtio.saving) });
        // Set code for the following accounts:
        //  - zdtio (code: zdtio.bios) (already set by tester constructor)
        //  - zdtio.msig (code: zdtio.msig)
        //  - zdtio.token (code: zdtio.token)
        // set_code_abi(N(zdtio.msig), contracts::zdtio_msig_wasm(), contracts::zdtio_msig_abi().data());//, &zdtio_active_pk);
        // set_code_abi(N(zdtio.token), contracts::zdtio_token_wasm(), contracts::zdtio_token_abi().data()); //, &zdtio_active_pk);

        set_code_abi(N(zdtio.msig),
                     contracts::zdtio_msig_wasm(),
                     contracts::zdtio_msig_abi().data());//, &zdtio_active_pk);
        set_code_abi(N(zdtio.token),
                     contracts::zdtio_token_wasm(),
                     contracts::zdtio_token_abi().data()); //, &zdtio_active_pk);

        // Set privileged for zdtio.msig and zdtio.token
        set_privileged(N(zdtio.msig));
        set_privileged(N(zdtio.token));

        // Verify zdtio.msig and zdtio.token is privileged
        const auto& zdtio_msig_acc = get<account_object, by_name>(N(zdtio.msig));
        BOOST_TEST(zdtio_msig_acc.privileged == true);
        const auto& zdtio_token_acc = get<account_object, by_name>(N(zdtio.token));
        BOOST_TEST(zdtio_token_acc.privileged == true);


        // Create SYS tokens in zdtio.token, set its manager as zdtio
        ...
        create_currency(N(zdtio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to zdtio.system
        issue(N(zdtio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(zdtio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "zdtio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), zdtio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), zdtio_assert_message_exception);
