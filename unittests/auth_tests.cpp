...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for zdtio reqauth action with alice's spending key
   chain.link_authority("alice", "zdtio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "zdtio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with zdtio reqauth
   chain.unlink_authority("alice", "zdtio", "reqauth");
   ...
   // Link authority for any zdtio action with alice's scud key
   chain.link_authority("alice", "zdtio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "zdtio", "first",  "reqauth");
...
try {
   ...
   // Creating account with zdtio. prefix with privileged account
   chain.create_account("zdtio.test1");

   // Creating account with zdtio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("zdtio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'zdtio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "zdtio", "zdtio.any", "reqauth" );
   chain.link_authority( "bob", "zdtio", "zdtio.any", "reqauth" );

   /// this should succeed because zdtio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = zdtio::chain::resource_limits::resource_usage_object;
   using by_owner = zdtio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link zdtio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
