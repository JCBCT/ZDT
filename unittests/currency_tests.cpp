...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(zdtio.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(zdtio.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::zdtio_token_abi().data()).as<abi_def>(), abi_serializer_max_time)
      {
         create_account( N(zdtio.token));
         set_code( N(zdtio.token), contracts::zdtio_token_wasm() );

         auto result = push_action(N(zdtio.token), N(create), mutable_variant_object()
                 ("issuer",       zdtio_token)
         ...
         result = push_action(N(zdtio.token), N(issue), mutable_variant_object()
                 ("to",       zdtio_token)
      ...
      static const std::string zdtio_token;
};

const std::string currency_tester::zdtio_token = name(N(zdtio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(zdtio.token), expected.get_symbol(), N(zdtio.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(zdtio.token), N(transfer), mutable_variant_object()
         ("from", zdtio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(zdtio.token), N(transfer), mutable_variant_object()
                                    ("from", zdtio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_overspend, currency_tester ) try {
   ...
   // Overspend!
   {
      ...
      BOOST_CHECK_EXCEPTION( push_action(N(alice), N(transfer), data),
                             zdtio_assert_message_exception, zdtio_assert_message_is("overdrawn balance") );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(zdtio_token, N(alice), "100.0000 CUR");
   ...
   {
      BOOST_REQUIRE_THROW(transfer(N(alice), N(carl), "20.50 USD"), zdtio_assert_message_exception);
   }
