...
class zdtio_token_tester : public tester {
public:

   zdtio_token_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(zdtio.token) } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = zdtio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(zdtio.token), acc, N(accounts), symbol_code );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(zdtio.token), N(create), mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(zdtio_token_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, zdtio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, zdtio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, zdtio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, zdtio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, zdtio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, zdtio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, zdtio_token_tester ) try {
