 ...
 */
#include "proxy.hpp"
#include <zdtio/transaction.hpp>

using namespace zdtio;

proxy::proxy( zdtio::name self, zdtio::name first_receiver, zdtio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      zdtio::token::transfer_action a( "zdtio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, zdtio::ignore<std::vector<char>> ) {
