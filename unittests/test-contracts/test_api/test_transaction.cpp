...
struct test_action_action {
   static zdtio::name get_account() {
      return zdtio::name{ACCOUNT};
   }

   static zdtio::name get_name() {
      return zdtio::name{NAME};
   }
...
void test_transaction::send_action_large() {
   ...
   act.send();
   zdtio_assert( false, "send_message_large() should've thrown an error" );
}
...
void test_transaction::test_tapos_block_prefix() {
   ...
   zdtio_assert( tbp == tapos_block_prefix(), "tapos_block_prefix does not match" );
}
...
void test_transaction::test_tapos_block_num() {
   ...
   zdtio_assert( tbn == tapos_block_num(), "tapos_block_num does not match" );
}
...
void test_transaction::test_read_transaction() {
   ...
   zdtio_assert( size == read, "read_transaction failed");
   h = zdtio::sha256(buf, read);
   print(h);
}
...
void test_transaction::test_transaction_size() {
   ...
   zdtio_assert( trans_size == transaction_size(), "transaction size does not match" );
}
...
void test_transaction::send_transaction_empty( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   zdtio_assert( false, "send_transaction_empty() should've thrown an error" );
}
...
void test_transaction::assert_false_error_handler( const zdtio::transaction& dtrx ) {
   zdtio_assert( dtrx.actions.size() == 1, "transaction should only have one action" );
   zdtio_assert( dtrx.actions[0].account == "testapi"_n, "transaction has wrong code" );
   zdtio_assert( dtrx.actions[0].name.value == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name" );
   zdtio_assert( dtrx.actions[0].authorization.size() == 1, "action should only have one authorization" );
   zdtio_assert( dtrx.actions[0].authorization[0].actor == "testapi"_n, "action's authorization has wrong actor" );
   zdtio_assert( dtrx.actions[0].authorization[0].permission == "active"_n, "action's authorization has wrong permission" );
}
...
void test_transaction::send_transaction_large( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   zdtio_assert( false, "send_transaction_large() should've thrown an error" );
}
...
void test_transaction::cancel_deferred_transaction_success() {
   ...
   zdtio_assert( (bool)r, "transaction was not found" );
}

void test_transaction::cancel_deferred_transaction_not_found() {
   using namespace zdtio;
   auto r = cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   zdtio_assert( !r, "transaction was canceled, whild should not be found" );
}
...
void test_transaction::send_cf_action_fail() {
   ...
   act.send_context_free();
   zdtio_assert( false, "send_cfa_action_fail() should've thrown an error" );
}

void test_transaction::stateful_api() {
   char buf[4] = {1};
   db_store_i64( zdtio::name{"testtrans"}.value, zdtio::name{"table"}.value, zdtio::name{"testtrans"}.value, 0, buf, 4 );
}
...
void test_transaction::repeat_deferred_transaction( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   trx.send( sender_id, zdtio::name{receiver} );
}
