...
void test_crypto::test_recover_key_assert_false() {
   ...
   read_action_data( (char*)&sh, sizeof(sh) );
   assert_recover_key( &sh.hash, (const char*)&sh.sig, sizeof(sh.sig), (const char*)&sh.pk, sizeof(sh.pk) );
   zdtio_assert( false, "should have thrown an error" );
}
...
void test_crypto::test_recover_key() {
   ...
   for ( uint32_t i=0; i < sizeof(pk); i++ )
      if ( pk.data[i] != sh.pk.data[i] )
         zdtio_assert( false, "public key does not match" );
}

void test_crypto::test_sha1() {
   zdtio::checksum160 tmp;

   tmp = zdtio::sha1( test1, my_strlen(test1) );
   zdtio_assert( my_memcmp((void *)test1_ok_1, &tmp, sizeof(zdtio::checksum160)), "sha1 test1" );

   tmp = zdtio::sha1( test3, my_strlen(test3) );
   zdtio_assert( my_memcmp((void *)test3_ok_1, &tmp, sizeof(zdtio::checksum160)), "sha1 test3" );

   tmp = zdtio::sha1( test4, my_strlen(test4) );
   zdtio_assert( my_memcmp((void *)test4_ok_1, &tmp, sizeof(zdtio::checksum160)), "sha1 test4" );

   tmp = zdtio::sha1( test5, my_strlen(test5) );
   zdtio_assert( my_memcmp((void *)test5_ok_1, &tmp, sizeof(zdtio::checksum160)), "sha1 test5" );
}

void test_crypto::test_sha256() {
  zdtio::checksum256 tmp;

  tmp = zdtio::sha256( test1, my_strlen(test1) );
  zdtio_assert( my_memcmp((void *)test1_ok_256, &tmp, sizeof(zdtio::checksum256)), "sha256 test1" );

  tmp = zdtio::sha256( test3, my_strlen(test3) );
  zdtio_assert( my_memcmp((void *)test3_ok_256, &tmp, sizeof(zdtio::checksum256)), "sha256 test3" );

  tmp = zdtio::sha256( test4, my_strlen(test4) );
  zdtio_assert( my_memcmp((void *)test4_ok_256, &tmp, sizeof(zdtio::checksum256)), "sha256 test4" );

  tmp = zdtio::sha256( test5, my_strlen(test5) );
  zdtio_assert( my_memcmp((void *)test5_ok_256, &tmp, sizeof(zdtio::checksum256)), "sha256 test5" );
}

void test_crypto::test_sha512() {
  zdtio::checksum512 tmp;

  tmp = zdtio::sha512( test1, my_strlen(test1) );
  zdtio_assert( my_memcmp((void *)test1_ok_512, &tmp, sizeof(zdtio::checksum512)), "sha512 test1" );

  tmp = zdtio::sha512( test3, my_strlen(test3) );
  zdtio_assert( my_memcmp((void *)test3_ok_512, &tmp, sizeof(zdtio::checksum512)), "sha512 test3" );

  tmp = zdtio::sha512( test4, my_strlen(test4) );
  zdtio_assert( my_memcmp((void *)test4_ok_512, &tmp, sizeof(zdtio::checksum512)), "sha512 test4" );

  tmp = zdtio::sha512( test5, my_strlen(test5) );
  zdtio_assert( my_memcmp((void *)test5_ok_512, &tmp, sizeof(zdtio::checksum512)), "sha512 test5" );
}

void test_crypto::test_ripemd160() {
  zdtio::checksum160 tmp;

  tmp = zdtio::ripemd160( test1, my_strlen(test1) );
  zdtio_assert( my_memcmp((void *)test1_ok_ripe, &tmp, sizeof(zdtio::checksum160)), "ripemd160 test1" );

  tmp = zdtio::ripemd160( test3, my_strlen(test3) );
  zdtio_assert( my_memcmp((void *)test3_ok_ripe, &tmp, sizeof(zdtio::checksum160)), "ripemd160 test3" );

  tmp = zdtio::ripemd160( test4, my_strlen(test4) );
  zdtio_assert( my_memcmp((void *)test4_ok_ripe, &tmp, sizeof(zdtio::checksum160)), "ripemd160 test4" );

  tmp = zdtio::ripemd160( test5, my_strlen(test5) );
  zdtio_assert( my_memcmp((void *)test5_ok_ripe, &tmp, sizeof(zdtio::checksum160)), "ripemd160 test5" );
}

void test_crypto::sha256_null() {
  zdtio::checksum256 tmp;
  tmp = zdtio::sha256( nullptr, 100);
  zdtio_assert( false, "should've thrown an error" );
}

void test_crypto::sha1_no_data() {
  zdtio::checksum160 tmp;

  tmp = zdtio::sha1( test2, my_strlen(test2) );
  zdtio_assert( my_memcmp((void *)test2_ok_1, &tmp, sizeof(zdtio::checksum160)), "sha1 test2" );
}

void test_crypto::sha256_no_data() {
  zdtio::checksum256 tmp;

  tmp = zdtio::sha256( test2, my_strlen(test2) );
  zdtio_assert( my_memcmp((void *)test2_ok_256, &tmp, sizeof(zdtio::checksum256)), "sha256 test2" );
}

void test_crypto::sha512_no_data() {
  zdtio::checksum512 tmp;

  tmp = zdtio::sha512( test2, my_strlen(test2) );
  zdtio_assert( my_memcmp((void *)test2_ok_512, &tmp, sizeof(zdtio::checksum512)), "sha512 test2" );
}

void test_crypto::ripemd160_no_data() {
  zdtio::checksum160 tmp;

  tmp = zdtio::ripemd160( test2, my_strlen(test2) );
  zdtio_assert( my_memcmp((void *)test2_ok_ripe, &tmp, sizeof(zdtio::checksum160)), "ripemd160 test2" );
}
...
void test_crypto::assert_sha256_false() {
  ...
  zdtio_assert( false, "should have failed" );
}
