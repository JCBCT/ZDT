...
CONTRACT test_ram_limit : public contract {
   ...
   using contract::contract;
      ...
      ACTION setentry( name payer, uint64_t from, uint64_t to, uint64_t size ) {
         ...
         zdtio::print("test_ram_limit::setentry ", zdtio::name{self}, "\n");
      ...
      ACTION rmentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         zdtio::print("test_ram_limit::rmentry ", zdtio::name{self}, "\n");
         test_table table( self, self.value );
         for ( int key = from; key <=to; ++key ) {
            auto itr = table.find(key);
            zdtio_assert ( itr != table.end(), "could not find test_table entry" );
            table.erase(itr);
         }
      }
      ...
      ACTION printentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         zdtio::print("test_ram_limit::printout ", zdtio::name{self}, ":");
         test_table table( self, self.value );
         for ( int key = from; key <= to; ++key ) {
            auto itr = table.find(key);
            zdtio::print("\nkey=", key);
            zdtio_assert ( itr != table.end(), "could not find test_table entry" );
            zdtio::print(" size=", itr->data.size());
         }
      }
   ...
   private:
      TABLE test {
         ...
         ZDTLIB_SERIALIZE( test, (key)(data) )
      };
      typedef zdtio::multi_index< "test.table"_n, test> test_table;
};

#pragma clang diagnostic pop

ZDTIO_DISPATCH( test_ram_limit, (setentry)(rmentry)(printentry) )
