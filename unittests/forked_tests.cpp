...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(zdtio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(zdtio.token), contracts::zdtio_token_wasm() );
   c.set_abi( N(zdtio.token), contracts::zdtio_token_abi().data() );
   c.produce_blocks(10);


   auto cr = c.push_action( N(zdtio.token), N(create), N(zdtio.token), mutable_variant_object()
              ("issuer",       "zdtio" )
   ...
   cr = c.push_action( N(zdtio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
