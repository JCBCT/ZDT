...
namespace zdtio_system {

class zdtio_system_tester : public TESTER {
public:

   zdtio_system_tester()
   : zdtio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   zdtio_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(zdtio.token), N(zdtio.ram), N(zdtio.ramfee), N(zdtio.stake),
               N(zdtio.bpay), N(zdtio.vpay), N(zdtio.saving), N(zdtio.names) });

      produce_blocks( 100 );

      set_code( N(zdtio.token), contracts::zdtio_token_wasm() );
      set_abi( N(zdtio.token), contracts::zdtio_token_abi().data() );

      {
         const auto& accnt = control->db().get<account_object,by_name>( N(zdtio.token) );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer_max_time);
      }

      create_currency( N(zdtio.token), config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( "zdtio" ) );

      set_code( config::system_account_name, contracts::zdtio_system_wasm() );
      set_abi( config::system_account_name, contracts::zdtio_system_abi().data() );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("zdtio")  + get_balance("zdtio.ramfee") + get_balance("zdtio.stake") + get_balance("zdtio.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& zdtin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",zdtin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(zdtio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(zdtio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(zdtio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = zdtio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(zdtio.token), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "zdtio_global_state", data, abi_serializer_max_time );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(zdtio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "zdtio", "zdtio.msig", core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( N(zdtio.msig), contracts::zdtio_msig_wasm() );
         set_abi( N(zdtio.msig), contracts::zdtio_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(zdtio.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      //stake more than 15% of total ZDT supply to activate chain
      transfer( "zdtio", "alice1111111", core_from_string("650000000.0000"), "zdtio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::zdtio_code_name}, 1},
...
inline uint64_t M( const string& zdt_str ) {
   return core_from_string( zdt_str ).get_amount();
}
